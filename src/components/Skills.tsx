          ...
          <div className="space-y-6">
            <Card title="Tech & Development" subtitle="Modern web & mobile">
              <div className="space-y-4">
                {tech.map((t) => <Bar key={t.name} {...t} />)}
              </div>
            </Card>
            <Card title="Hardware & Embedded" subtitle="IoT & circuit design">
              <div className="space-y-4">
                {hardware.map((t) => <Bar key={t.name} {...t} />)}
              </div>
            </Card>
          </div>
          ...
          <div className="space-y-6">
            <Card title="Key Strengths">
              <div className="grid sm:grid-cols-2 gap-3">
                {strengths.map((s) => (
                  <div key={s} className="rounded-xl border border-border bg-secondary/40 px-4 py-3 text-sm text-center">
                    {s}
                  </div>
                ))}
              </div>
            ...
            <Card title="Interests & Hobbies">
              <div className="flex flex-wrap gap-2">
                {interests.map((s) => (
                  <span key={s} className="rounded-full border border-border bg-secondary/40 px-4 py-2 text-sm">
                    {s}
                  </span>
                ))}
              </div>
            ...
            <Card title="Professional Experience">
              <p className="text-muted-foreground text-sm leading-relaxed">
                ...
                Open to collaborations, internships, and freelance work in IoT, hardware integration,

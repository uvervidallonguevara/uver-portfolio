...
const codeRepos = [
  { name: "severino-perfumes", desc: "E-commerce site for a boutique perfume brand.", lang: "TypeScript", stars: 12 },
  { name: "sched-it", desc: "Venue scheduling & availability platform.", lang: "React", stars: 8 },
  { name: "flood-warning", desc: "Arduino-based early flood detection system.", lang: "C++", stars: 15 },
  { name: "portfolio", desc: "This very portfolio — built with React + TanStack.", lang: "TypeScript", stars: 3 },
];

const hardwareItems = [
  { name: "Flood Warning Device", parts: ["Arduino UNO", "Ultrasonic HC-SR04", "Buzzer", "GSM SIM800L"] },
...
const ideas = [
  { title: "AI-powered Mental Health Companion", note: "On-device journaling + sentiment trends. Private by default." },
  { title: "Barangay Disaster Mesh", note: "LoRa nodes that share flood / fire alerts without internet." },
  { title: "Smart Jeepney Tracker", note: "GPS + crowd-sourced ETA for PUVs in metro areas." },
  { title: "Study-Mode Wearable", note: "Haptic nudge when focus drops — paired with a Pomodoro app." },
];
...
function CodeView() {
  return (
    <div className="font-mono text-sm space-y-3">
      {codeRepos.map((r) => (
        <div key={r.name} className="rounded-lg border border-border bg-secondary/30 p-4 hover:border-primary transition">
...
function HardwareView() {
  return (
    <div className="space-y-4">
      {hardwareItems.map((h) => (
        <div key={h.name} className="rounded-lg border border-dashed border-primary/40 bg-secondary/20 p-4">
          ...
          <div className="relative grid grid-cols-2 sm:grid-cols-4 gap-2">
            {h.parts.map((p) => (
              <div key={p} className="relative rounded border border-border bg-background/60 px-3 py-2 text-xs font-mono text-center">
          ...
          <div className="mt-3 text-[10px] text-muted-foreground font-mono tracking-widest">── wired & tested ──</div>
        </div>
      ))}
    </div>
  );
      ...
      {toolStack.map((g) => (
        <div key={g.group} className="rounded-lg border border-border bg-secondary/30 p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {g.items.map((t) => (
              <span key={t} className="text-xs px-2.5 py-1 rounded-md bg-background border border-border">
                {t}
              </span>
            ))}
          </div>
  ...
  return (
    <div className="space-y-3">
      {ideas.map((i, idx) => (
        ...
        >
          <span className="absolute left-3 top-3 text-xs font-mono text-muted-foreground">{String(idx + 1).padStart(2, "0")}</span>
